...
public class ViewItemNormalizer extends AcceptorNormalizer<SojEvent, GlobalEvent> {
...
//when event.page_id in (2056016, 2058306, 2349624) then 'VI_LAYER_PAGE_IDS'
    ...
    public void update(int code, SojEvent src, GlobalEvent tar) {
        tar.setEventPrimaryAsset(EventPrimaryAsset.VIEW_ITEM_PAGE);
//        tar.setSubPageType(SubPageType.other.toString());
        ...
        putIfNotBlankStr(tar.getBizDtl(), Field.F_META_CATEGORY_ID, ExtractTag.extract(tar.getPayload(), Tag.T_META));
        putIfNotBlankStr(tar.getBizDtl(), Field.F_L1_CATEGORY_ID, ExtractTag.extract(tar.getPayload(), Tag.T_L1));
        putIfNotBlankStr(tar.getBizDtl(), Field.F_L2_CATEGORY_ID, ExtractTag.extract(tar.getPayload(), Tag.T_L2));
        putIfNotBlankStr(tar.getBizDtl(), Field.F_LEAF_CATEGORY_ID, ExtractTag.extract(tar.getPayload(), Tag.T_LEAF));
        ...
        putIfNotBlankStr(tar.getBizDtl(), Field.F_BIDDERS, ExtractTag.extract(tar.getPayload(), Tag.T_BDRS));
        putIfNotBlankStr(tar.getBizDtl(), Field.F_CURRENT_PRICE, ExtractTag.extract(tar.getPayload(), Tag.T_CUR_PRICE));
        putIfNotBlankStr(tar.getBizDtl(), Field.F_CURRENT_PRICE_USD, ExtractTag.extract(tar.getPayload(), Tag.T_CP_USD));
        ...
        putIfNotBlankStr(tar.getBizDtl(), Field.F_REMAINING_QUANTITY, ExtractTag.extract(tar.getPayload(), Tag.T_VI_QTY_REMAIN));
        ...
        putIfNotBlankStr(tar.getBizDtl(), Field.F_SELLER_FEEDBACK_SCORE, ExtractTag.extract(tar.getPayload(), Tag.T_FB_SCORE));
        ...
        putIfNotBlankStr(tar.getBizDtl(), Field.F_BID_COUNT, ExtractTag.extract(tar.getPayload(), Tag.T_BC));

        // BIN
        putIfNotBlankStr(tar.getBizDtl(), Field.F_BIN_AMT, ExtractTag.extract(tar.getPayload(), Tag.T_BIN_AMT));

        // PL
        putIfNotBlankStr(tar.getBizDtl(), Field.F_PROMOTED_LISTING_FLG, ExtractTag.extract(tar.getPayload(), Tag.T_PROMO_L));
    }
