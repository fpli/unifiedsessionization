...
@Data
public class UbiKafkaConsumerConfig extends KafkaCommonConfig {

    private List<String> topics;

    private UbiKafkaConsumerConfig(DataCenter dc) {
        super(dc);
    }

    public static UbiKafkaConsumerConfig ofDC(DataCenter dataCenter) {
        UbiKafkaConsumerConfig config = new UbiKafkaConsumerConfig(dataCenter);
        final List<String> topics = FlinkEnvUtils.getList(KAFKA_CONSUMER_TOPIC_UBI);
        Preconditions.checkState(CollectionUtils.isNotEmpty(topics));
        config.setTopics(topics);
        return config;
    }

    public static UbiKafkaConsumerConfig ofDC(String dataCenter) {
    ...
    public String getBrokersForDC(DataCenter dc) {
        String propKey = KAFKA_CONSUMER_BOOTSTRAP_SERVERS_UBI + "." + dc.getValue().toLowerCase();
